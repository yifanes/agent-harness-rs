...
//!
//! 1. **In-memory table** — populated by a background fetch of
...
struct CatalogState {
    /// model_id (lowercased) -> limits, sourced from models.dev.
...
/// Serialize back to the models.dev `{provider:{model:{limit:{...}}}}`
/// envelope so `load_disk_cache_into_memory` can read our own cache.
/// Note: we always emit a single synthetic provider `cached` whose
/// `models` map holds every entry — that round-trips through the same
/// `ProviderEntry { models }` deserializer.
